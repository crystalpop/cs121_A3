...
Our index structure:
Index = {
            'word1': [{docID: [frequency, [positions], tf-idf]}, {docID2: [frequency, [positions], tf-idf ]}]
}
"""
...
class Indexer:
        ...
        for root, _ , files in os.walk(self.data_folder):
            for file in files:
                if file.endswith(".json"):
                    ...
                    tokens, doc_hash = self.clean_text(raw_html)
    ...
    def clean_text(self, raw_html: str) -> list:
        ...
        doc_hash = hashlib.sha256(" ".join(tokens).encode()).hexdigest()

        stems = []
        stemmer = stem.PorterStemmer()
        for token in tokens:
            stems.append(stemmer.stem(token))
        return stems, doc_hash 
    ...
    def build_inverted_index(self, tokens: list, doc_id: int):
        ...
        for position, word in enumerate(tokens):
            if word not in self.inverted_index:
                ...
                self.inverted_index[word].append({doc_id: [1, [position]]}) # first posting
            else:
                ...
                if next(iter(last_posting)) == doc_id:
                    ...
                    self.inverted_index[word].insert(pos, {doc_id: [1, [position]]})
